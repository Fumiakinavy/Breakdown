...
### 概要
Inbox/Active/Completedの各タスク選択で詳細シートが開くようにし、`TaskDetailSheet`を表示できるよう連携した。
### 目的
ユーザーがタスクをタップした際に詳細化ポップアップを利用できるようにする。
### 最適な実装方法
行を`Button`化してタスクIDを保持し、`sheet(item:)`で詳細シートを提示する構造に更新。
### 最適と思われる理由
SwiftUIのシートプレゼンテーションに沿った実装で、タスクID経由でビュー・モデルの最新状態を参照できる。
### 入出力
入力: タップされた`Task` ID。  
出力: `TaskDetailSheet`の表示。
### 実装の評価
要望どおりタップでポップアップが開き、以後の編集機能拡張にも備えられる。評価: 96/100。
